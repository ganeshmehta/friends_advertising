...
export default function Home() {
  ...
  return (
    <>
      ...
      <section className="relative w-full h-screen flex flex-col items-center justify-start pt-32 overflow-hidden">
        ...
        >
          <motion.div
            ...
            className="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-[var(--neon-blue)]/30 bg-[var(--neon-blue)]/5 text-[var(--neon-blue)] text-sm font-medium mb-6"
          >
            <Sparkles className="w-4 h-4" />
            <span>Welcome to the Future of OOH Advertising</span>
          </motion.div>
          ...
          >
            We don't just print banners. We engineer massive, unmissable brand experiences across Mumbai and Satara.
          </motion.p>
        </motion.div>

        {/* The 3D Billboard - Pushed up to interact with text */}
        <motion.div 
          initial={{ opacity: 0, y: 100 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ delay: 0.6, duration: 1 }}
          className="z-20 w-full flex-1 min-h-[50vh] relative -mt-10"
        >
          <Billboard3D />
        </motion.div>
      </section>
      ...
      <div className="w-full bg-[var(--neon-purple)] py-4 overflow-hidden border-y border-[var(--neon-purple)]/50 relative z-30 shadow-[0_0_30px_rgba(188,19,254,0.3)]">
        <motion.div 
          animate={{ x: [0, -1035] }}
          transition={{ ease: "linear", duration: 15, repeat: Infinity }}
          className="whitespace-nowrap flex gap-8 items-center text-black font-black text-2xl tracking-widest"
        >
          {Array(8).fill("THE COMPLETE OUTDOOR SOLUTION • UNMISSABLE IMPACT • ").map((text, i) => (
